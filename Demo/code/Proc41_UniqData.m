...
for I_Uniq = 1: numel(UniqName)
    ...
    TepDLAI(:,I_Uniq) = mean(TepLAI,2);
    TepDRg(:,I_Uniq) = mean(TepRg,2);
    TepDPre(:,I_Uniq) = mean(TepPre,2);
    TepDTmn(:,I_Uniq) = mean(TepTmn,2);
    TepDTa(:,I_Uniq) = mean(TepTa,2);
    TepDVPD(:,I_Uniq) = mean(TepVPD,2);
    TepDPho(:,I_Uniq) = mean(TepPho,2);
