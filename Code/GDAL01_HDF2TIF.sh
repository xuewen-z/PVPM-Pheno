...
# number cycles
    OuFile=$(echo $InFile | sed 's/hdf/tif/g' | sed 's/MCD12Q2/MCD12NUM/g') ;
    OuName=$OuDir/$Year/$OuFile;

    echo "gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:NumCycles -co @COMPRESS=LZW@ $OuName";
    gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:NumCycles -b 1 -co "COMPRESS=LZW" $OuName &
...
#    gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:Greenup -b 1 -co "COMPRESS=LZW" $OuName &
...
#    gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:Maturity -b 1 -co "COMPRESS=LZW" $OuName &
...
#    gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:Senescence -b 1 -co "COMPRESS=LZW" $OuName &
...
#    gdal_translate HDF4_EOS:EOS_GRID:"$InName":MCD12Q2:Dormancy -b 1 -co "COMPRESS=LZW" $OuName &
